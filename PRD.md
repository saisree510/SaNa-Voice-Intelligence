...
| C6 - Voice/text progressive architecture generation | In progress | The backend now applies accepted semantic canvas operations to the authoritative current Blueprint after durable event persistence, so progressive `add_node` and `connect_nodes` batches advance saved architecture state. The LiveKit voice agent has a `create_overview_architecture_tool` that creates an owned Overview Architecture and submits deterministic validated operation batches from a finalized title/specification, with tests for backend payload shape, internal-agent authentication headers and progressive operation ordering. Backend auth now falls back to Supabase `auth.get_user()` when legacy JWT-secret verification does not match newer Supabase signing-key sessions, preventing valid browser sessions from receiving false 401s. Production browser verification passed after Railway redeploy: a signed-in user created an architecture, posted a `soul_agent` `add_node` event, and read back the `web` component from `current_blueprint.components`. Flutter Web now includes an authenticated ArchitectureService that fetches the latest owned architecture, converts its saved Blueprint into deterministic canvas operations and sends it through the versioned same-origin iframe bridge; the React/Excalidraw canvas accepts `soul.canvas.load_blueprint` and renders generated components/connections instead of relying only on the static mock. A follow-up canvas hotfix made the embedded message listener stable so animation progress no longer re-emits `ready`, asks Flutter to resend the Blueprint and resets the drawing loop. The client now ignores single-node smoke-test architectures when choosing what to display, requiring at least 2 components or 1 connection to hydrate saved blueprints, preferring richer diagrams ranked by component and connection count, and safely falling back to the full demo blueprint when no richer saved architecture exists. Verification passed for unit tests (`architecture_service_test.dart`), `dart format`, `flutter analyze`, `flutter test` (17 tests passing), and production web build. Remaining C6 work: stream new generation events into the already-open visible canvas in real time, surface Drawing/Saved/Review states in Flutter, and verify the full browser conversation to visible canvas path. |
