...
| B5 - Resume and download verification | In verification | A completed project, its history and generated-file metadata persisted through browser refresh and re-login. The web client now fetches the signed ZIP and triggers a first-party browser download instead of relying on a popup; public download and two-account access verification remain. |
