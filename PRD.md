...
| C3 - Embed canvas in the Soul conversation | In progress | Flutter Web now builds the standalone canvas into `mobile/web/canvas` and embeds it as a same-origin iframe in the active Agent conversation screen. Desktop starts conversation-first with the canvas collapsed, and `View on canvas` opens a Canvas Focus Mode where the diagram becomes the main stage with `Back to conversation` and `Ask Soul` controls. Canvas Focus hides the compact voice orb/top reservation and the normal top overlay so the workspace can use the available vertical space and focus controls receive clicks. The `Ask Soul` control is an explicit drawer-open action, while the bottom chat control exits Canvas Focus back to conversation first; clicking it again from conversation returns to the orb. The chat drawer sits beside the iframe rather than over it so controls remain clickable in Flutter Web. The prior resizable split remains available from canvas controls, while narrow layouts use Conversation/Canvas tabs and preserve the existing LiveKit voice controls. Flutter and the embedded canvas exchange a versioned JSON `postMessage` handshake, canvas state, acknowledgements, safe rejections and command messages for Fit, Replay, Pause and Play, scoped to the current origin. |
