...
| D7 - Build/canvas stabilization | In progress | Browser testing confirmed deployed Build Mode can now create downloadable files. Build Mode now creates one draft architecture as soon as the user has a concrete project idea, updates that same saved Blueprint when a material requirement changes, announces the same `architecture_id` through LiveKit so Flutter refetches and redraws it, and links that draft to the final Build Project instead of creating a second default diagram. Follow-up fixes hydrate the approved Architecture Blueprint into `BuildSpec`, load canvas state by exact `architecture_id`, reconnect the canvas WebSocket when the active architecture changes, prevent stale/global architectures and repeated chat cards, recover a linked architecture when a room packet is missed, render the dotted workspace in both loaded and empty states, and keep project creation resilient if architecture persistence fails. Automated verification: backend 61 tests and voice-agent 8 tests pass locally. Deployment verification remains: deploy the updated LiveKit agent, then create a new Build Mode conversation and verify the initial draft and a later scope change redraw the same canvas. |
