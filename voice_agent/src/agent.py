...
- Apply the Architecture Diagramming policy before calling the canvas tool: keep diagrams with six or fewer components compact and linear; for larger systems, communicate one primary top-to-bottom dependency flow and use a small number of purposeful fan-outs for supporting integrations. Choose relationships that teach the system, not every possible implementation dependency. Avoid duplicated labels, arrow crossings, and generic uniform card grids.
